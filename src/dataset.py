...
class CorpusDataset(Dataset):
    ...
    def batchify(self, batch_samples):
        ...
        for raw, rv, rv_intervals in batch_samples:
            ...
            masked_word_idxs = list(
                np.random.choice(list(range(len(rv_intervals))), mask_word_num, False)
            )
        ...
        INPUT_IDS = torch.LongTensor(
            pad_batch_seqs(INPUT_IDS, self.tokenizer.vocab["[PAD]"])
        )
...
def get_data_loaders(rv_path, lb_path, tokenizer, batch_size, val_split=0.15):
    ...
    train_loader = DataLoader(
        train_data,
        batch_size,
        collate_fn=full_dataset.batchify,
        shuffle=True,
        num_workers=5,
        drop_last=False,
    )
...
def get_full_data_loaders(
    rv_path, lb_path, tokenizer, batch_size, type="makeup", shuffle=False
):
...
def get_data_loaders_cv(
    ...
    for train_idx, val_idx in folds:
        train_loader = DataLoader(
            [full_dataset.samples[i] for i in train_idx],
            batch_size,
            collate_fn=full_dataset.batchify,
            shuffle=True,
            num_workers=5,
        ...
        val_loader = DataLoader(
            [full_dataset.samples[i] for i in val_idx],
            batch_size,
            collate_fn=full_dataset.batchify,
            shuffle=False,
            num_workers=5,
...
def get_aug_data_loaders_cv(
    rv_path, lb_path, tokenizer, batch_size, type="makeup", folds=5
):
    # full_dataset = ReviewDataset(rv_path, lb_path, tokenizer, type)
    ...
    for train_idx, val_idx in folds:
        ...
        val_loader = DataLoader(
            val_dataset,
            batch_size,
            collate_fn=val_dataset.batchify,
            shuffle=False,
            num_workers=5,
            drop_last=False,
        )
...
def get_data_loaders_round2(tokenizer, batch_size, val_split=0.15):
    makeup_rv1 = ReviewDataset(
        "../data/TRAIN/Train_reviews.csv", "../data/TRAIN/Train_labels.csv", tokenizer
    )
    makeup_rv2 = ReviewDataset(
        "../data/TRAIN/Train_makeup_reviews.csv",
        "../data/TRAIN/Train_makeup_labels.csv",
        tokenizer,
    )
    makeup_rv = ConcatDataset([makeup_rv1, makeup_rv2])
    laptop_rv = ReviewDataset(
        "../data/TRAIN/Train_laptop_reviews.csv",
    ...
    corpus_loader = DataLoader(
        corpus_rv,
        batch_size,
        collate_fn=laptop_corpus1.batchify,
        shuffle=True,
        num_workers=5,
        drop_last=False,
    )
    ...
    makeup_train_loader = DataLoader(
        makeup_train,
        batch_size // 2,
        collate_fn=makeup_rv1.batchify,
        shuffle=True,
        num_workers=5,
        drop_last=False,
    )
    ...
    torch.manual_seed(502)
    laptop_train, laptop_val = random_split(
        laptop_rv, [laptop_train_size, laptop_val_size]
    )
    laptop_train_loader = DataLoader(
        laptop_train,
        batch_size // 2,
        collate_fn=laptop_rv.batchify,
        shuffle=True,
        num_workers=5,
        drop_last=False,
    )
...
def get_pretrain2_loaders(tokenizer, batch_size, val_split=0.15):
    ...
    laptop_rv = ReviewDataset(
        "../data/TRAIN/Train_laptop_corpus.csv",
        "../data/TRAIN/Train_laptop_corpus_labels.csv",
        tokenizer,
        "laptop",
    )
    laptop_val_rv = ReviewDataset(
        "../data/TRAIN/Train_laptop_reviews.csv",
        "../data/TRAIN/Train_laptop_labels.csv",
        tokenizer,
        "laptop",
    ...
    laptop_val_loader = DataLoader(
        laptop_val_rv,
        batch_size,
        collate_fn=laptop_val_rv.batchify,
        shuffle=False,
        num_workers=5,
        drop_last=False,
    )
...
def get_pretrain_2_laptop_fake_loaders_cv(tokenizer, batch_size, folds=5):
    ...
    for cv_idx in range(folds):
        laptop_rv_big = ReviewDataset(
            "../data/TRAIN/Train_laptop_corpus.csv",
            "../data/TRAIN/Train_laptop_corpus_labels" + str(cv_idx) + ".csv",
            tokenizer,
            "laptop",
        )
        ...
        train_loader = DataLoader(
            laptop_rv_big,
            batch_size,
            collate_fn=laptop_rv_big.batchify,
            shuffle=True,
            num_workers=5,
            drop_last=False,
        )

...
if __name__ == "__main__":
    ...
    cv_loaders, val_idxs = get_data_loaders_cv(
    ...
    for cv_idx, (train_loader, val_loader) in enumerate(cv_loaders):
        if not os.path.exists(
            "../models/" + model_config["name"] + "_cv" + str(cv_idx)
        ):
            continue
        ...
        model.load_state_dict(
            torch.load("../models/" + model_config["name"] + "_cv" + str(cv_idx))
        )
        model.cuda()
        model.eval()
        try:
    ...
    result.to_csv(
        "../testResults/" + model_config["name"] + ".csv", header=True, index=False
    )

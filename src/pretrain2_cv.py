...
from dataset import (
    get_pretrain_2_laptop_fake_loaders_cv,
...
if __name__ == "__main__":
    ...
    makeup_loader, makeup_val_loader, corpus_loader = get_pretrain2_loaders_cv(
        tokenizer, batch_size=args.bs
    )

    laptop_gt_cv_loaders = get_data_loaders_cv(
    ...
    laptop_fake_cv_loaders = get_pretrain_2_laptop_fake_loaders_cv(
        tokenizer, batch_size=args.bs
    )
    BEST_THRESHS = [0.1] * 5
    BEST_F1 = [0] * 5
    for cv_idx, (laptop_fake_train) in enumerate(laptop_fake_cv_loaders):
        ...
        model = OpinioNet.from_pretrained(
            model_config["path"], version=model_config["version"]
        )
        model.cuda()
        ...
        scheduler = GradualWarmupScheduler(
            optimizer,
            total_epoch=2
            * max(len(makeup_loader), len(laptop_fake_train), len(corpus_loader)),
        )
        best_val_f1 = 0
        ...
        for e in range(EP):
            ...
            print(
                "makeup_train: loss %.5f, f1 %.5f, pr %.5f, rc %.5f"
                % (makeup_loss, makeup_f1, makeup_pr, makeup_rc)
            )
            print(
                "laptop_train: loss %.5f, f1 %.5f, pr %.5f, rc %.5f"
                % (laptop_loss, laptop_f1, laptop_pr, laptop_rc)
            )
            print("lm loss %.5f", total_lm_loss)

            print("Epoch [%d/%d] makeup eval:" % (e, EP))
            val_loss, val_f1, val_pr, val_rc, best_th = eval_epoch(
                model, makeup_val_loader, type="makeup"
            )
            print(
                "makeup_val: loss %.5f, f1 %.5f, pr %.5f, rc %.5f, thresh %.2f"
                % (val_loss, val_f1, val_pr, val_rc, best_th)
            )

            print("Epoch [%d/%d] laptop eval:" % (e, EP))
            val_loss, val_f1, val_pr, val_rc, best_th = eval_epoch(
                model, laptop_gt_val, type="laptop"
            )
            print(
                "laptop_val: loss %.5f, f1 %.5f, pr %.5f, rc %.5f, thresh %.2f"
                % (val_loss, val_f1, val_pr, val_rc, best_th)
            )

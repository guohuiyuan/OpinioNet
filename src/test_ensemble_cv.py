...
if __name__ == "__main__":
    ...
    tokenizer = BertTokenizer.from_pretrained(
        PRETRAINED_MODELS["roberta"]["path"], do_lower_case=True
    )
    cv_loader, val_idxs = get_data_loaders_cv(
    ...
    print(
        "f1 %.5f, pr %.5f, rc %.5f, th %.5f" % (best_f1, best_pr, best_rc, best_thresh)
    )
    ...
    result.to_csv(
        "../testResults/" + "ensemble_result_label_" + ("%.5f" % best_f1) + ".csv",
        header=True,
        index=False,
    )

...
def gen_submit(ret, raw):
    ...
    for i, opinions in enumerate(ret):
        ...
        for j, (opn, score) in enumerate(opinions):
            ...
            P = ID2P[p]
            result = result.append(
                {"id": cur_idx, "A": A, "O": O, "C": C, "P": P}, ignore_index=True
            )
        cur_idx += 1
    return result
...
if __name__ == "__main__":
    ...
    test_loader = DataLoader(
        test_dataset, 12, collate_fn=test_dataset.batchify, shuffle=False, num_workers=5
    )
    ...
    result.to_csv(
        "../submit/ensemble-" + str(round(time.time())) + ".csv",
        header=False,
        index=False,
    )

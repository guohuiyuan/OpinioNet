...
from openai import OpenAI  # 替换为同步客户端
from tqdm import tqdm      # 替换为同步 tqdm
...
def get_extraction(client, text, row_id, progress_file):
    # 构造用户输入
...
def main(args):
    ...
    if unprocessed_df.empty:
        ...
        # 串行循环处理
        for _, row in tqdm(unprocessed_df.iterrows(), total=len(unprocessed_df)):
            text = str(row[text_col])
            row_id = str(row[id_col])
            get_extraction(client, text, row_id, progress_file)
